...
import colorsys
from .imvizmpl import ImageVizMPL
import os
...
def figure2image(fig, load_mpl=True):
    """
    ...
    if(load_mpl):
        # lazy import to make sure that other functions in this file works without importing matplotlib
...
def figure2image_save(fig, file_name=None, bbox_inches='tight', dpi=None):
    """

    :param fig: matplotlib figure object
    :param file_name: if file name is not provided, it just returns image as np array
    :param bbox_inches:
    :param dpi:
    :return:
    """
    if(file_name is None):
        temp_file=os.tempnam()+'.png'
    else:
        temp_file = file_name
    fig.savefig(temp_file, bbox_inches=bbox_inches, dpi= dpi)
    im = np.asarray(Image.open(temp_file))
    return im
...
def get_heatmap_multiple(images):
    """
    :param images: NxH,W
    :return:
    """
    ...
    for im in images:
        hm=mplv.get_heatmap(im)
        hms.append(np.expand_dims(hm,0))

    return np.vstack(hms)
...
def display_side2side(im, mask, ngrid_y, ngrid_x):
    """
    Display image and mask side by side
    :param im: Nx Hx W x c
    :param mask:Nx Hx Wx c
    :return:
    """

    assert (im.shape[0] * 2 <= ngrid_y * ngrid_x), 'Invalid grid assignment '

    x_disp = im
    result = np.zeros((x_disp.shape[0] * 2, x_disp.shape[1], x_disp.shape[2], x_disp.shape[3]))
    result[::2] = x_disp * 255
    result[1::2] = mask * 255

    n_blank = ngrid_x * ngrid_y - result.shape[0]

    assert (n_blank <= ngrid_x and n_blank <= ngrid_y), ' too many blank grids, ' + str(n_blank)

    if n_blank > 0:
        ss = list(result.shape)
        ss[0] = n_blank
        blank = np.zeros(tuple(ss))
        result = np.vstack([result, blank])

    result = stack_patches(result, ngrid_y, ngrid_x)
...
def display_side2sidev1(im, mask, ngrid_y, ngrid_x):
    """
    Display image and mask side by side
    :param im: Nx Hx W x c
    :param mask:Nx Hx Wx c
    :ngrid_y  number of ygrids to puts im+mask in
    :ngrid_x  number of xgrids to puts im+mask in
    :return:
    """

    im_mask = np.concatenate([ensure3channels(im), ensure3channels(mask)], axis=2)
    result = stack_patches(im_mask, ngrid_y, ngrid_x)
    result = np.squeeze(result)
    return result
